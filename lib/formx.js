...
export default function initFormX() {
  ...
  (function () {
    const intro = $("intro"),
      word = intro.querySelector(".word"),
      sub = intro.querySelector(".sub"),
      sq = intro.querySelector(".sq"),
      btn = $("xbtn"),
    ...
    word.animate(
      ...
      { duration: 600, delay: 150, easing: E, fill: "both" }
    );
    sq.animate(
      [
      ...
      { duration: 620, delay: 560, easing: "cubic-bezier(.3,1.5,.4,1)", fill: "both" }
    );
    sub.animate(
      [
      ...
      { duration: 700, delay: 820, easing: E, fill: "both" }
    );
    const t1 = setTimeout(() => {
      if (killed) return;
      word.animate(
        [{ opacity: 1 }, { opacity: 0, transform: "translateX(40px) scale(.6)" }],
        { duration: 480, easing: E, fill: "both" }
      );
      sub.animate([{ opacity: 1 }, { opacity: 0, transform: "scale(.6)" }], {
        duration: 400,
        easing: E,
        fill: "both",
      });
  ...
  (function () {
    ...
    on(
      ...
      () => {
        ...
        c.style.strokeDashoffset = L * (1 - (max ? scrollY / max : 0));
      },
      { passive: true }
    );
  })();
  ...
  /* =========================================================
     ...
     ========================================================= */
  const VIDEO_SRC = "/vedios/apartment_super_hd.mp4";
  (function () {
    const hero = $("vhero"),
      film = $("film"),
      sim = $("sim"),
    ...
    if (VIDEO_SRC) {
      ...
      on(film, "loadedmetadata", () => (ready = true));
      if (coarse) {
        film.loop = true;
        film.play().catch(() => {});
      }
    ...
    function frame() {
      if (stopped) return;
      shown += (p - shown) * 0.25;
      if (VIDEO_SRC && ready && !coarse && film.duration)
        film.currentTime = shown * (film.duration - 0.05);
      else if (!VIDEO_SRC) room(shown);
    ...
    function build() {
      ...
      for (let y = 0; y < Math.ceil(h / gap); y++)
        for (let x = 0; x < Math.ceil(w / gap); x++)
          pts.push({ x: x * gap + gap / 2, y: y * gap + gap / 2, a: 0 });
    }
    function draw() {
      if (stopped) return;
      t += 0.011;
      ...
      for (const p of pts) {
        const dx = px - p.x,
          dy = py - p.y,
          d = Math.hypot(dx, dy),
          pull = Math.exp(-d / (gap * 6.5));
        const wave = Math.sin(p.x * 0.006 + p.y * 0.009 + t) * 0.5;
        p.a += (Math.atan2(dy, dx) * pull + wave * (1 - pull) - p.a) * 0.09;
        const c = Math.cos(p.a) * len,
          s = Math.sin(p.a) * len;
        ctx.strokeStyle = "rgba(242,104,41," + (0.09 + pull * 0.8) + ")";
        ctx.lineWidth = 1 + pull * 1.7;
        ctx.beginPath();
