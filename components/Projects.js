  ...
  useEffect(() => {
    ...
    function group(id, fn) {
      $(id).addEventListener('click', (e) => {
        ...
        scrollTo({ top: $('stage').offsetTop, behavior: 'smooth' });
      });
    }
    ...
    function swap(i) {
      ...
      setTimeout(() => {
        buildOv(cur); pos = 0; paintOv(); ov.classList.add('rev');
        setTimeout(() => autoTo(1, 2200), 230);
      }, 210);
    }
    $('close').addEventListener('click', closeOv);
    $('prev').addEventListener('click', () => swap(cur - 1));
    $('next').addEventListener('click', () => swap(cur + 1));
    ...
    tline.addEventListener('pointerdown', (e) => {
      cancelAnimationFrame(autoR); drag = true;
      ...
      const io = new IntersectionObserver((es) => es.forEach((e) => {
        ...
        [[$('t1'), P.length, ''], [$('t2'), 100, '%'], [$('t3'), 100, '%']].forEach(([el, to, suf]) => {
          ...
          (function s(t) {
            ...
            el.textContent = Math.round(to * e2) + suf; if (k < 1) requestAnimationFrame(s);
          })(t0);
        });
      }), { threshold: .4 });
      io.observe($('pwtrust'));
        ...
        <div className="stick">
          ...
          <div className="wallwrap" id="wallwrap">
            <svg className="wave" viewBox="0 0 1200 220" preserveAspectRatio="none" aria-hidden="true">
              <defs>
                <linearGradient id="pwg" x1="0" x2="1">
                  <stop offset="0" stopColor="#F26829" stopOpacity="0" />
                  <stop offset=".5" stopColor="#F26829" stopOpacity=".22" />
                  <stop offset="1" stopColor="#F26829" stopOpacity="0" />
                </linearGradient>
              </defs>
              <path id="wv1" fill="none" stroke="url(#pwg)" strokeWidth="1.6" d="" />
              <path id="wv2" fill="none" stroke="url(#pwg)" strokeWidth="1" d="" opacity=".6" />
            </svg>
            <div id="wall"></div>
          </div>
